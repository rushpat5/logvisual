...
if uploaded_file:
    ...
    fig2 = px.pie(bot_hits, names="bot_normalized", values="hits",
                  title="Traffic Share by Bot", hole=0.4)
    ...
    st.markdown("### 🔢 HTTP Status Distribution Across Bots")
    status_summary["status"] = pd.to_numeric(status_summary["status"], errors="coerce")
    status_summary = status_summary.dropna(subset=["status"])
    ...
    bot_choice = st.selectbox("Select a bot for detailed URL and status breakdown",
                              sorted(df["bot_normalized"].unique()))
    df_bot = df[df["bot_normalized"] == bot_choice]
    st.write(f"**{bot_choice}** — {len(df_bot):,} hits, {df_bot['url'].nunique()} unique URLs")

    df_bot_summary = (df_bot.groupby(["url", "status"]).size()
                      .reset_index(name="hits").sort_values("hits", ascending=False))
    df_bot_summary["hits"] = df_bot_summary["hits"].clip(lower=0)  # fix negative bug
    ...
    st.download_button(f"⬇️ Download {bot_choice} Breakdown",
                       csv_export, f"{bot_choice.lower()}_breakdown.csv", "text/csv")
else:
